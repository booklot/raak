...
<?php $a = array( 'style'=>'color: #2BA6CB;' );
	...
	$mp = 'style = "padding: 15px 7px; margin-bottom:10px; text-decoration:none; color: #FFF;
				font-weight:bold; display:block; text-align:center; background-color: #1daced!important; margin-top:4px;"';
	$mp1 = 'style = "padding: 15px 7px; margin-bottom:10px; text-decoration:none; color: #FFF;
	...
	<tr>
		...
		<td <?php echo $container;?> bgcolor="#FFFFFF">
			...
			<table>
				<tr>
					<td>
						<h3 <?php echo $h3;?>>Hello <?php echo $invoice['fname'];?></h3>
						<p class="lead">You have successfully placed an order on <?php echo lang('site-name');?>, Zimbabwe's online shopping mall</p>
...
<tfoot>
	...
	<tr>
		<td colspan="1">Discount Coupon</td>
		<td colspan="2"><em><?php echo $invoice['coupon_name'];?></em> </td>
		<td style="text-align:right;">-<?php echo toc( $invoice['discount'] )?></td>
	</tr>
	<tr>
		<td colspan="4"><br clear="all"></td>
	</tr>
	...
	<tr>
		...
		<td colspan="1" style="text-align:right;font-weight:bold;"><?php echo toc( $invoice['total'] )?></td>
						...
						<p>
						Due to high demand for some products, we will reserve the products you ordered for a period of 24 hours from the invoice issue date. If you do not make a payment within this period, your ordered products may become unavailable.
<br>
We have attached a copy of your invoice.
</p>
...
<td>
<h5 <?php echo $h5;?>>
	<?php echo anchor( base_url('account/make_payment/'.$invoice['invoice_num']), 'Make Payment', $mp1 );?>
</h5>
</td>
<td></td>
<td>
<h5 <?php echo $h5;?>>
	<?php echo anchor( base_url('account/invoice/'.$invoice['invoice_num']), 'View Order', $mp );?>
</h5>
</td>
</tr>
</table>
