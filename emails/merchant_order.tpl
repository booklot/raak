...
<?php $a = array( 'style'=>'color: #2BA6CB;' );
	...
	$mp = 'style = "padding: 15px 7px; margin-bottom:10px; text-decoration:none; color: #FFF;
				font-weight:bold; display:block; text-align:center; background-color: #1daced!important; margin-top:4px;"';
	$ao = 'style = "padding: 15px 7px; margin-bottom:10px; text-decoration:none; color: #FFF;
				font-weight:bold; display:block; text-align:center; background-color: #1daced!important; margin-top:4px;"';
	$ro = 'style = "padding: 15px 7px; margin-bottom:10px; text-decoration:none; color: #FFF;
				font-weight:bold; display:block; text-align:center; background-color: #DB4A39!important; margin-top:4px;"';
		...
		<td style="width:100%;display: block!important;max-width: 600px!important;margin: 0 auto!important;clear: both!important;" >
				...
				<table bgcolor="#e7e7e7" style="width:100%">
					<tr>
						...
						<td align="right"><h6 <?php echo $h6collapse;?>>Order Confirmation</h6></td>
					</tr>
				</table>
				</div>
	...
	<tr>
		...
		<td <?php echo $container;?> bgcolor="#FFFFFF">
			...
			<table>
				<tr>
					<td>
						<h3 <?php echo $h3;?>>You have a received a new order!</h3>
...
<?php $orders = $invoice['invoice_details'];
	...
	<tr>
		<td rowspan="2"><?php $img = array( 'src'=>"images/products/100x75/{$order['image']}" );
			$link = "shops/{$order['cid']}/".url_title($order['shop'],'-',TRUE)."/products/{$order['id']}/".url_title($order['name'],'-',TRUE);
			echo anchor( home_url($link), img($img) );?></td>
...
<?php if(0>1):?>	
	<tr>
		<td style="border-top:none;border-bottom:none;"><?php echo $order['name'];?></td>
		<td style="border-top:none;border-bottom:none;"><?php echo $order['qty'];?></td>
		<td style="border-top:none;border-bottom:none;"><?php echo toc($order['price']*$order['qty']);?></td>
	</tr>
<?php endif;?>
	<tr>
		<td colspan="3" style="text-align:left;padding: 0px 5px 5px 20px;border-top:none;">
			<small><?php echo array_implode( ' : ', ', ', unserialize($order['options']) );?></small>
		</td>
	</tr>
...
<tfoot>
	...
	<tr>
		...
		<td colspan="1" style="text-align:right;font-weight:bold;"><?php echo toc( $total )?></td>
	</tr>
	<tr>
		<td colspan="3">Transaction Fees</td>
		<td style="text-align:right;"><?php echo toc( $fees )?></td>
	</tr>
	<tr>
		<td colspan="3">Your Balance</td>
		<td style="text-align:right;"><?php echo toc( $total - $fees )?></td>
	</tr>
						...
						<p>
						To avoid inconveniences, we require you to confirm product availabity before the customer makes a payment.
</p>
...
<td>
<h5 <?php echo $h5;?>>
	<?php echo anchor( home_url('account/merchant_order/'.$invoice['invoice_num'].'/1'), 'Accept Order', $ao );?>
</h5>
</td>
<td></td>
<td>
<h5 <?php echo $h5;?>>
	<?php echo anchor( home_url('account/merchant_order/'.$invoice['invoice_num'].'/2'), 'Reject Order', $ro );?>
</h5>
</td>
</tr>
</table>
	...
	<!-- Callout Panel -->
	<p <?php echo $p_callout;?>>If you have any questions about this order, you can email our sales team, 
	<?php echo mailto('sales@zimall.co.zw', 'sales@zimall.co.zw', $a_callout);?> or call <?php echo lang('site-phone');?></p>
	<!-- /Callout Panel -->					
	...
	<tr>
		...
		<td <?php echo $container?>>
				...
				<tr>
					<td align="center">
						<p style="margin-bottom: 10px;font-weight: normal;font-size: 14px;line-height: 1.6;">
							<?php echo anchor( home_url('shop/terms_conditions'), 'Terms and Conditions', $a);?> |
							<?php echo anchor( home_url('shop/privacy'), 'Privacy Policy', $a);?> |
							<?php echo anchor( home_url('shop/delivery'), 'Delivery Information', $a);?> |  
							<?php echo anchor( home_url('shop/merchant_policy'), 'Merchant Policy', $a );?>
						</p>
					</td>
				</tr>
			</table>
