	...
	<div class="block block-info">
			...
			<div class="btn-toolbar">
				<?php 
					if( $invoice['amount_paid'] < $invoice['total'] && $invoice['status'] < 4 )
						echo anchor('orders/make_payment/'.$invoice['invoice_num'],'Make Payment','class="btn btn-xs btn-primary"');
					elseif( $invoice['status'] == 6 )
						echo anchor( 'orders/make_payment/'.$invoice['invoice_num'], 'Confirm Delivery', 
						'class="btn btn-xs btn-success"' );
					elseif( $invoice['amount_paid']==$invoice['total'] && $invoice['shipping_method']=='Download' )
						echo anchor( 'orders/downloads/', 'Go To My Downloads', 
						'class="btn btn-xs btn-success"' );
					
					echo anchor( base_url().'orders/printable_invoice/'.$invoice['invoice_num'], 
					...
					echo anchor( site_url("orders/invoice/{$invoice['invoice_num']}").'#addInvoiceNote', 
					...
					if(!$ohp) $ohp = 'orders/order_history';
		...
		<div class="invoice col-xs-12">
			...
			<div class="col-sm-7 pull-left">
				...
				<h4><?php echo anchor( site_url("orders/invoice/{$invoice['invoice_num']}").'#addInvoiceNote', 'Invoice Notes:', 
			...
			<div class="pagers">
				<div class="btn-toolbar">
					<?php 
						...
						echo anchor( current_url().'#cancel_order', 'Cancel Order', 
							['class'=>"btn btn-default btn-sm", 'data-toggle'=>'modal' ] );
						if( $invoice['amount_paid'] < $invoice['total'] && $invoice['status'] < 4 )
						echo anchor( 'orders/make_payment/'.$invoice['invoice_num'], 'Make Payment', 'class="btn btn-primary btn-sm"' );
						elseif( $invoice['status'] == 6 )
						echo anchor( 'orders/make_payment/'.$invoice['invoice_num'], 'Confirm Delivery', 
...
<div class="modal fade" id="addInvoiceNote">
	<div class="modal-dialog">
		...
		<?php echo form_open( site_url("orders/invoice/{$invoice['invoice_num']}"), 
...
<div class="modal fade" id="cancel_order">
	<div class="modal-dialog">
		<div class="modal-content">
	<?php echo form_open( "orders/cancel_order/{$invoice['invoice_num']}", [ 'class'=>'form-horizontal', 'style'=>'margin:0;' ] );?>
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
